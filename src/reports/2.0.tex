...
\begin{shdd}
    ...
    Вероятность попадания в доверительный интервал = доверительная вероятность \(\beta\):
    \[
    P(\hat{\theta} - \delta < \theta < \hat{\theta} + \delta) = \beta,
    \]
    отношение \(\frac{t_{\gamma}s}{\sqrt{n}} = \beta\) называется точностью оценки. 
    \begin{itemize}
...
\begin{problem}
    ...
    \begin{solution}
        ...
        \begin{enumerate}
            ...
            (больше \(n\rightarrow\) меньше \(\beta\rightarrow\) - у`же интервал)
        \end{enumerate}
    \end{solution}
